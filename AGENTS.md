...
- Nomes de campo exatos do backend: `speed_wpm` (nunca `speed`); `correct` nunca é enviado pelo cliente; exercícios `key_capture` enviam `press_durations` + `input_method` (nunca `user_answer` junto). `input_method` é a tecla configurada ou o literal `"Touch"` (captura por toque — constante em `services/practice.service.ts`, aceita pelo backend fora da whitelist).
