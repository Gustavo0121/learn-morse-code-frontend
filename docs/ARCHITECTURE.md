# Arquitetura — Learn Morse Code Frontend

Documento técnico de referência: decisões de arquitetura, contratos com a API e checklists de qualidade/segurança. Para visão geral e setup, ver o [README](../README.md).
...
**Ambientes e URL da API**: por design, existe um único `environment.ts` com `apiUrl: '/api'` (relativo) para todos os ambientes — em dev o proxy resolve (`proxy.conf.json` → backend Django em `http://localhost:8000`, mantendo cookies em contexto same-origin), em produção um rewrite `/api/*` no host estático faz proxy para a API, mantendo o navegador em origem única (requisito do cookie `SameSite=Strict`). Não há URLs de API por ambiente para configurar; a única variável de build é `NODE_VERSION=22` no host.
...
- **Rollback**: o dashboard do Render permite voltar para qualquer deploy anterior (aba Deploys → Rollback), sem novo build.
